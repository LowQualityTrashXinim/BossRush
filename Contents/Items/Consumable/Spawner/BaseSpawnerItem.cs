...
namespace BossRush.Contents.Items.Consumable.Spawner {
	public abstract class BaseSpawnerItem : ModItem {
		...
		public override void SetStaticDefaults() {
			...
			for (int i = 0; i < NPCtypeToSpawn.Length; i++) 				NPCID.Sets.MPAllowedEnemies[NPCtypeToSpawn[i]] = true;
		...
		public override bool? UseItem(Player player) {
			if (player.whoAmI == Main.myPlayer) {
				...
				for (int i = 0; i < NPCtypeToSpawn.Length; i++) 					if (Main.netMode != NetmodeID.MultiplayerClient) 						if (UseSpecialSpawningMethod) 							SpecialSpawningLogic(player);
						else 							NPC.SpawnOnPlayer(player.whoAmI, NPCtypeToSpawn[i]);
					else 						NetMessage.SendData(MessageID.SpawnBossUseLicenseStartEvent, number: player.whoAmI, number2: NPCtypeToSpawn[i]);
			}
			return true;
		}
	}
