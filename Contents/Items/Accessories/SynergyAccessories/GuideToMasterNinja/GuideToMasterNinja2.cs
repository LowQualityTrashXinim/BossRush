	...
	internal class GuideToMasterNinja2 : SynergyModItem {
		...
		public override void ModifySynergyToolTips(ref List<TooltipLine> tooltips, PlayerSynergyItemHandle modplayer) {
			...
			if (player.GetModPlayer<PlayerNinjaBook>().GuidetoMasterNinja2 && (player.HasItem(ItemID.Shuriken) || player.HasItem(ItemID.ThrowingKnife) || player.HasItem(ItemID.PoisonedKnife))) {
				...
				if (player.HasItem(ItemID.BoneDagger)) 					tooltips.Add(new TooltipLine(Mod, "BoneDaggerGTMN2", $"[i:{ItemID.BoneDagger}] Shuriken gain 5 flat damage increase"));
				if (player.HasItem(ItemID.FrostDaggerfish)) 					tooltips.Add(new TooltipLine(Mod, "BoneDaggerGTMN2", $"[i:{ItemID.FrostDaggerfish}] Shuriken gain 5 flat damage increase"));
			}
			...
			if (player.GetModPlayer<PlayerNinjaBook>().GuidetoMasterNinja) 				tooltips.Add(new TooltipLine(Mod, "FinalMaster", $"[i:{ModContent.ItemType<GuideToMasterNinja>()}] You summon a ring of shuriken and knife"));
		}
