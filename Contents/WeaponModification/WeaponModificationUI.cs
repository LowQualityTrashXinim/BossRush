			...
			if (player.TryGetModPlayer(out WeaponModificationPlayer modplayer)) {
				foreach (var element in Elements) {
					if (element is WeaponModificationUIslot wmUISlot && wmUISlot != null) {
						if (wmUISlot.ModificationType != -1 && wmUISlot.item == null)
							modplayer.WeaponModification_inventory[wmUISlot.WhoAmI] = wmUISlot.ModificationType;
					}
				}
			}
		}
		...
		private void SelectState() {
			if (item == null) {
				if (WeaponModificationSystem.SelectedInventorySlot != -1) {
					...
					modplayer.WeaponModification_inventory[WeaponModificationSystem.SelectedInventorySlot] = modplayer.WeaponModification_inventory[WhoAmI];
					modplayer.WeaponModification_inventory[WhoAmI] = cache;
					WeaponModificationSystem.SelectedInventorySlot = -1;
					return;
				}
				...
				if (WeaponModificationSystem.SelectedInventorySlot == WhoAmI) {
					WeaponModificationUIslot UIslot =
						(WeaponModificationUIslot)Parent.Children
						.Where(e => e is WeaponModificationUIslot { item: not null } slot && slot.WhoAmI == WeaponModificationSystem.SelectedModifySlot)
						.FirstOrDefault();
					WeaponModificationPlayer modplayer = player.GetModPlayer<WeaponModificationPlayer>();
					if (UIslot == null)
						return;
				...
				if (WeaponModificationSystem.SelectedModifySlot != -1) {
					...
					if (item.TryGetGlobalItem(out WeaponModificationGlobalItem globalItem)) {
						...
						globalItem.ModWeaponSlotType[WeaponModificationSystem.SelectedModifySlot] = globalItem.ModWeaponSlotType[WhoAmI];
						globalItem.ModWeaponSlotType[WhoAmI] = cache;
					}
					WeaponModificationSystem.SelectedModifySlot = -1;
					return;
				}
